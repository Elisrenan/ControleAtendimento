...
def exibir_atendimentos():
    ...
    for atendimento in atendimentos:
        lista_resultados.insert(
            tk.END,
            f"Atendimento | ID: {atendimento[0]} | "
            f"Pessoa: {atendimento[1]} | "
            f"Descrição: {atendimento[2]} | "
            f"Status: {atendimento[3]}"
        )
...
def acao_atualizar_status():
    ...
    sucesso, mensagem = atualizar_status_atendimento(
        atendimento_id,
        novo_status
    )
...
tk.Button(
    frame_busca,
    text="Buscar",
    command=acao_buscar_pessoa
).grid(row=0, column=2, padx=5)

tk.Button(
    frame_busca,
    text="Listar Pessoas",
    command=exibir_pessoas
